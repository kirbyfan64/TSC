...
void cThromp::Set_Image_Dir(fs::path dir)
{
    ...
    if (!File_Exists(pResource_Manager->Get_Game_Pixmap(path_to_utf8(dir / utf8_to_path("/up.settings")))) && !File_Exists(pResource_Manager->Get_Game_Pixmap(path_to_utf8(dir /  utf8_to_path("/up.png"))))) {
    ...
    if (collision->m_direction == m_direction) {
        ...
        if (m_state == STA_FLY) {
            ...
            // kill enemy
                ...
                gp_hud->Add_Points(enemy->m_kill_points, m_pos_x + m_image->m_w / 3, m_pos_y - 5, "", static_cast<uint8_t>(255), 1);
                enemy->DownGrade(1);
